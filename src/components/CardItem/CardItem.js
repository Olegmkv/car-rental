...
import { StyledLi, StyledSpan} from "./CardItem.styled"
import { selectFavorites } from "../../redux/selectors"
import { useEffect, useState } from "react"
import { addFavorit, removeFavorit } from "../../redux/favoritSlice"

export const CardItem = ({ card }) => {
    const favorites = useSelector(selectFavorites);
    const dispatch = useDispatch();
    
    const [favoritIndex, setFaforitIndex] = useState(
        favorites.findIndex(el => el.id === card.id)
    );

    useEffect(() => {
        setFaforitIndex(favorites.findIndex(el => el.id === card.id));
    }, [favorites, card]);
    
    const handleChangeFavorites = () => {
        if (favoritIndex === -1) {
            dispatch(addFavorit(card))
        } else {
            dispatch(removeFavorit(favoritIndex));
        }
    };
    
    return (
        <StyledLi>
            <StyledSpan>Card</StyledSpan>
            <StyledSpan>{card.model}</StyledSpan>
            <button type="button"
                onClick={handleChangeFavorites}
                style={{  backgroundColor: 'transparent', border: 'none', cursor: 'pointer' }}>
                <svg width="18" height="18" viewBox="0 0 18 18" fill={favoritIndex === -1 ? "none" : "blue"} xmlns="http://www.w3.org/2000/svg">
